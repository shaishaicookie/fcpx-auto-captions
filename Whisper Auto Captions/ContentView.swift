...
struct ContentView: View {
    ...
    @State var status = "Spliting audio file by every 10 minutes···"
               ...
               let selectedLanguageShortCut = languagesMapping[selectedLanguage]
            {
                ...
                if selectedLanguageShortCut != "zh" {
                    ...
                    if selectedLanguage == "Chinese Simplified" {
                        task.arguments = ["-m", modelPath.path, "-l", selectedLanguageShortCut, "-pp", "-osrt", "-f", outputWavFilePath, "--prompt", "\"以下是普通话的句子\""]
                    } else {
                        task.arguments = ["-m", modelPath.path, "-l", selectedLanguageShortCut, "-pp", "-osrt", "-f", outputWavFilePath, "--prompt", "\"以下是普通話的句子\""]
                    }
                }
            ...
            VStack() {
                HStack{
                    ...
                    Button(action: {
                        ...
                        self.status = "Spliting audio file by every 10 minutes···"
                    }) {
                ...
                Text("Current Status: \(self.status)").font(.title2)
                    .padding()
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .padding(.bottom, -28)
